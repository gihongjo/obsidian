...
- OSFP (Open Short Path First) 최단 경로 우선 프로토콜
    ...
    - Link State Routing 프로토콜 기반의 LSDB(Link State DataBase)를 구축하고 Dijkstra 알고리즘(최단 경로 알고리즘)을 이용하여 패킷이 목적지로 도달하기까지 최소 경로를 제공하는 라우팅 방식
        - 경로상 라우터의 개수: 홉 수
...
- DVRP(Distance Vecter Routing Protocol) 거리 백터 라우팅 프로토콜
    ...
    - 거리벡터 알고리즘의 필수 정보
        ...
        - 거리 벡터:
            ...
            - 전체 네트워크에 소속된 개별 네트워크들 까지의 거리정보를 관리함
            - 전체 네트워크 수가 5면 보관값고 5개임
        - 다음 홉 벡터
            - 개별 네트워크로 가기위한 다음 홉 정보
            - 다음 홉 벡터 H(x)=[홉(1), 홉[2] ... 홉[n]]

- RIP (Routing Information Protocol) 프로토콜
	- 이웃 라우터가 주기적으로 주는 정보에 의존해 거리 방향만으로 라우팅하는 방식
		- 기본적으로 이웃라우터가 제공하는 거리 벡터 정보가 짧은 시간 내에 도착해야 함
	- 라우팅 테이블에 적용하는데 제한을 둠
		- 거리 벡터 정보가 새로우면 라우팅 테이블에 추가
		- 거리 벡터 정보가 기존보다 좋으면 적용
		- 거리 벡터 정보가 다음 홉으로 등록하는 정보가 있으면 수정
	- RIP프로토콜의 패킷 구조
	- ![[Pasted image 20231129214035.png]]
		- Command: 1(RIP 요청)/2(RIP 응답)
		- Version: RIP 프로토콜의 버전
		- Address Family Identifier: 2(IP 프로토콜)
		- IP Address: xxx.xxx.xxx.0 
		- Metric: 목적지 네트워크까지의 거리

- 링크 상태 라우팅 프로토콜
	- 거리벡터 라우팅 프로토콜은 라우팅 정보가 주기적으로 전달되지만, 링크 상태 라우팅 프로토콜은 이웃 라우터의 변화가 있을 때 만 라우팅 정보가 전달됨

- 외부 라우팅 프로토콜
	- Inter-AS간 라우팅 프로토콜, Exteriror Gateway Protocol이라고도 함
		- 이전의 OSPF, RIP 프로토콜은 내부 라우팅용으로 개발됨
	- Path Vector Routing 
